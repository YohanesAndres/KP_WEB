...
<div class="top-title no-space" style="margin-bottom:-15px">
  ...
  <div >
    <div class="text-white text-center text-title">Form Tambah Muat Bongkar</div>
  </div>
</div>
<hr>
...
<div class=formmarg>
    <form action="{{ url('muat_bongkar/store/') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-9">
                <input type="number" name="uang_jalan" id="uang_jalan" class="form-control" placeholder="Masukkan Uang Jalan">
                @error('uang_jalan')
                <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
